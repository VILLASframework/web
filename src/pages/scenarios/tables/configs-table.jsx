...
const ConfigsTable = ({ scenario, ics }) => {
  ...
  const [isEditSignalMappingModalOpened, setIsEditSignalMappingModalOpened] =
    useState(false);
  ...
  const getBothSignals = async (configID) => {
    try {
      const resIn = await triggerGetSignals({
        configID: configID,
        direction: "in",
      }).unwrap();
      const resOut = await triggerGetSignals({
        configID: configID,
        direction: "out",
      }).unwrap();
      setSignals((prevSignals) => ({
        ...prevSignals,
        [configID]:
          resIn.signals.concat(resOut.signals).length > 0
            ? resIn.signals.concat(resOut.signals)
            : [],
      }));
    } catch (err) {
  ...
  const handleConfigCheck = (config, event) => {
    if (!checkedConfigsIDs.includes(config.id)) {
      ...
      setCheckedConfigsIDs((prevState) =>
  ...
  return (
    <div>
      ...
      <Table data={configs} allRowsChecked={false}>
        <CheckboxColumn
          enableCheckAll
        ...
        <DataColumn title="Name" dataKey="name" width={250} />
        <ButtonColumn
          ...
          modifier={(configId) => (
            <span>{getNumberOfSignalsModifier(configId, "out")}</span>
          )}
        />
        <ButtonColumn
          title="# Input Signals"
          ...
          locked={scenario.isLocked}
          modifier={(configId) => (
            <span>{getNumberOfSignalsModifier(configId, "in")}</span>
          )}
        />
        <ButtonColumn
