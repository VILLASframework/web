  ...
  const handleAddScenarioMapping = async (newMapping) => {
    if (newMapping) {
      try {
        ...
        const oldMappings =
          usergroup.scenarioMappings.length > 0
            ? [...usergroup.scenarioMappings]
            : [];
        await updateUsergroup({
          usergroupID: usergroupID,
          usergroup: {
            name: usergroup.name,
            scenarioMappings: [...oldMappings, newMapping],
          },
        }).unwrap();
        refetch();
      } catch (err) {
  ...
  const handleRemoveScenarioMapping = async (isConfirmed) => {
    if (isConfirmed) {
      try {
        ...
        const newMappings = [...usergroup.scenarioMappings].filter(
          (mapping) => mapping.id !== mappingToDelete.id
        );
        await updateUsergroup({
          usergroupID: usergroupID,
          usergroup: { name: usergroup.name, scenarioMappings: newMappings },
        }).unwrap();
        refetch();
      } catch (error) {
    ...
    <div className="section">
      ...
      <Table data={usergroup.scenarioMappings}>
        ...
        <DataColumn
          title="Name"
          dataKey="scenarioID"
          modifier={(scenarioID) => getScenarioName(scenarioID)}
        />
        <DataColumn
          title="Duplicated"
          dataKey="duplicate"
          modifier={(duplicate) => getDuplicateLabel(duplicate)}
        />
        <ButtonColumn
          width="200"
          align="right"
      ...
      <AddScenarioMappingDialog
        isDialogOpened={isAddScenarioDialogOpen}
        mappings={usergroup.scenarioMappings}
        onClose={(newMapping) => handleAddScenarioMapping(newMapping)}
      />
      <DeleteDialog
