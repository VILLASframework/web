...
const UsersTable = (props) => {
  ...
  const { data, refetch: refetchUsers } = useGetUsersOfScenarioQuery(
    scenario.id
  );
  const [addUserToScenario, { isSuccess: isUserAdded }] =
    useAddUserToScenarioMutation();
  const [removeUserFromScenario, { isSuccess: isUserRemoved }] =
    useRemoveUserFromScenarioMutation();
  ...
  const addUser = async () => {
    if (usernameToAdd.trim() === "") {
      notifyError("Please, enter correct username");
    } else {
      try {
        await addUserToScenario({
  ...
  return (
    <div>
      ...
      <Table data={users}>
        ...
        <DataColumn title="Name" dataKey="username" width={300} />
        <DataColumn title="Role" dataKey="role" width={100} />
        <ButtonColumn
          title=""
          width={30}
          align="right"
