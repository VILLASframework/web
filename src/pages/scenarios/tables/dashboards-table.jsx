...
const DashboardsTable = ({ scenario }) => {
  const { data: fetchedDashboards, refetch: refetchDashboards } =
    useGetDashboardsQuery(scenario.id);
  ...
  const [
    triggerGetWidgets,
    { isLoading: isWidgetsLoading, data: widgets, error: widgetsError },
  ] = useLazyGetWidgetsQuery();
  ...
  const copyDashboard = async (dashboardToCopy) => {
    ...
    try {
      const widgetsResponse = await triggerGetWidgets(
        dashboardToCopy.id
      ).unwrap();
      let parsedWidgets = [];
      if (widgetsResponse.widgets.length > 0) {
        parsedWidgets = JSON.parse(
          JSON.stringify(
            widgetsResponse.widgets.filter(
              (w) => w.dashboardID === parseInt(dashboardToCopy.id, 10)
            )
          )
        );
  ...
  const exportDashboard = async (dashboard) => {
    try {
      ...
      const fileName = dashboardToExport.name
        .replace(/\s+/g, "-")
        .toLowerCase();
      const blob = new Blob([JSON.stringify(dashboardToExport, null, 2)], {
    ...
    <div>
      ...
      <Table data={dashboards}>
        ...
        <LinkColumn
          ...
          width={300}
        />
        <DataColumn title="Grid" dataKey="grid" width={100} />
