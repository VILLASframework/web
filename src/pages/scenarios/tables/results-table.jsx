...
const ResultsTable = (props) => {
  ...
  const [isResultConfigDialogOpened, setIsResultConfigDialogOpened] =
    useState(false);
  ...
  const handleDownloadAllFiles = async (resultIndex) => {
    ...
    if (result && result.resultFileIDs && result.resultFileIDs.length > 0) {
      ...
      zip
        ...
        .catch((err) => {
          notifyError("Failed to create ZIP archive");
          console.error("Failed to create ZIP archive", err);
        });
    } else {
      notifyError("No files to download for the selected result");
    }
  };
  ...
  const modifyResultNoColumn = (id, result) => {
    return (
      <Button
        variant="link"
        style={{ color: "#047cab" }}
        onClick={() => openResultConfigSnapshots(result)}
      >
        {id}
      </Button>
    );
  ...
  const openResultConfigSnapshots = (result) => {
    if (
      result.configSnapshots === null ||
      result.configSnapshots === undefined
    ) {
    ...
    <div>
      ...
      <Table data={results}>
        ...
        <DataColumn
          title="Created at"
          dataKey="createdAt"
          modifier={(createdAt) => timeAgo(createdAt)}
          width={200}
          columnType="data"
        />
        <DataColumn
          title="Last update"
          dataKey="updatedAt"
          modifier={(updatedAt) => timeAgo(updatedAt)}
          width={200}
          columnType="data"
        />
        <LinkbuttonColumn
