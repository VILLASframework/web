...
class EditWidgetPlotColorsControl extends Component {
  ...
  componentDidUpdate() {
    ...
    if (
      JSON.stringify(this.state.signalIDs) !==
      JSON.stringify(prevState.signalIDs)
    ) {
      ...
      if (lineColorsChanged) {
        this.props.handleChange({
          target: { id: this.props.controlId, value: tempLineColors },
        });
      }
    }
  }
  ...
  editLineColor = (index) => {
    if (this.state.selectedIndex !== index) {
      let color =
        typeof this.state.lineColors[index] === "undefined"
          ? schemeCategory10[index % 10]
          : this.state.lineColors[index];
      this.setState({
        selectedIndex: index,
      ...
      <Form.Group style={this.props.style}>
        ...
        <div>
          {this.props.widget.signalIDs.map((signalID, idx) => {
            let color =
              typeof this.state.lineColors[idx] === "undefined"
                ? schemeCategory10[idx % 10]
                : this.state.lineColors[idx];
            ...
            let signal = this.props.signals.find(
            ...
            return (
              <OverlayTrigger
                ...
                placement={"bottom"}
                overlay={
                  <Tooltip id={'tooltip-${"signal name"}'}>
                    {signalNameTooltip}
                  </Tooltip>
                }
              >
