...
import { timeAgo, getDurationTimeString } from "../../utils/time-utils";
...
const ICCategoryTable = (props) => {
  ...
  const modifyUptimeColumn = (uptime, component) => {
    if (uptime >= 0) {
      return <span>{getDurationTimeString(uptime)}</span>;
    } else {
  ...
  if (props.category == "manager") {
    tableData = ics.filter(
      (ic) =>
        ic.category == props.category &&
        isGenericDisplayed ** (ic.type == "generic")
    );
  } else {
  ...
  useEffect(() => {
    if (tableData.length > 0) {
      for (let ic in tableData) {
        if (
          !checkedICs.hasOwnProperty(tableData[ic].id) &&
          !isLocalIC(tableData[ic])
        ) {
          setCheckedICs((prevState) => ({
            ...prevState,
            [tableData[ic].id]: false,
          }));
        }
      }
    }
  ...
  return (
    <div>
      ...
      <Table data={tableData}>
        ...
        />
        <DataColumn title="Type" dataKeys={["type"]} />
        <DataColumn
          title="Uptime"
          dataKey="uptime"
          modifier={(uptime, component) =>
        ...
        <DataColumn
          ...
          modifier={(uptime, component) => timeAgo(uptime)}
        />
        ...
        {currentUser.role === "Admin" ? (
          <ButtonColumn
            ...
            onEdit={(index) => {
              dispatch(openEditModal(tableData[index]));
            }}
            onExport={(index) => exportIC(index)}
            onDelete={(index) => {
              dispatch(openDeleteModal(tableData[index]));
            }}
          />
        ) : (
          <ButtonColumn
