...
const Scenarios = (props) => {
  const { data, refetch: refetchScenarios } = useGetScenariosQuery();
      ...
      try {
        ...
        if (error.data) {
          notificationsDataManager.addNotification(
            NotificationsFactory.LOAD_ERROR(err.data.message)
          );
        } else {
          notificationsDataManager.addNotification(
  ...
  return (
    <div className="section">
      ...
      <Table data={scenarios}>
        ...
        {currentUser.role === "Admin" ? (
          <ButtonColumn
            ...
            isLocked={(index) => {
              return scenarios[index].isLocked;
            }}
          />
        ) : (
          <></>
        ...
        <ButtonColumn
          ...
          onExport={(index) => {
            onExportScenario(index);
          }}
          onDuplicate={(index) => {
            onScenarioDuplicate(index);
          }}
          isLocked={(index) => {
            return scenarios[index].isLocked;
          }}
      ...
      <ImportScenarioDialog show={false} onClose={(data) => {}} nodes={null} />
