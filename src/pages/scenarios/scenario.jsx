...
const Scenario = (props) => {
  ...
  } = useGetScenarioByIdQuery(scenarioId);
  const scenario = fetchedScenarios?.scenario;

  const {
    data: fetchedICs,
    isLoading: areICsLoading,
    error,
    refetch: refetchICs,
  } = useGetICSQuery(scenarioId);
  ...
  if (isScenarioLoading) {
    ...
    const tooltip = scenario.isLocked
    ...
    return (
      <div className="section">
        ...
        {areICsLoading ? (
          ...
          <ConfigsTable ics={ics} scenario={scenario} />
        )}

        <DashboardsTable scenario={scenario} />

        <ResultsTable scenario={scenario} />

        <UsersTable scenario={scenario} />
      </div>
    );
  }
};
