...
const ICActionBoard = ({ externalICs }) => {
  ...
  const checkedICsIds = useSelector(
    (state) => state.infrastructure.checkedICsIds
  );
  ...
  const onReset = () => {
    ...
    checkedICsIds.forEach((id) => {
      dispatch(
        sendActionToIC({ token: sessionToken, id: id, actions: newAction })
      );
    });
  };
  ...
  const onRecreate = () => {
    ...
    for (let checkedIC of checkedICsIds) {
      ...
      if (IC) {
        ...
        if (manager) {
          newAction["parameters"] = IC.properties
            ? IC.properties
            : IC.statusupdateraw.properties;
          dispatch(
            sendActionToIC({
              token: sessionToken,
              id: manager.id,
