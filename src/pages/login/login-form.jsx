...
const LoginForm = ({ config }) => {
  ...
  const authenticateUser = async () => {
    try {
      const res = await authenticationRequest({
        mechanism: "internal",
        username: username,
        password: password,
      });
  ...
  const villasLogin = (
    <Form key="login_a">
      ...
      <Form.Group style={{ paddingTop: 15, paddingBottom: 5 }}>
        <Col>
          ...
          <Button
            variant="link"
            size="sm"
            style={{ marginLeft: 85, textDecoration: "none" }}
      ...
      <RecoverPassword
        ...
        onClose={() => setForgottenPassword(false)}
        config={config}
      />
    </Form>
  );
  ...
  if (config) {
    let externalLogin = _.get(config, [
      "authentication",
      "external",
      "enabled",
    ]);
    let provider = _.get(config, [
      "authentication",
      "external",
      "provider_name",
    ]);
    let url =
      _.get(config, ["authentication", "external", "authorize_url"]) +
      "?rd=/login/complete";
    ...
    if (externalLogin && provider && url) {
      return [
        villasLogin,
        <hr key="login_b" />,
        <Button key="login_c" onClick={(e) => (window.location = url)} block>
