...
class CustomTable extends Component {
  ...
  static addLabelCell(data, child) {
    ...
    if (labelKey && data[labelKey] != null) {
      ...
      cell.push(
        <span>
          &nbsp;
          <Badge bg={labelStyle[0]} className={labelStyle[1]}>
            {labelContent}
          </Badge>
        </span>
      );
  ...
  static addCheckbox(data, index, child) {
    let isDisabled =
      child.props.checkboxDisabled != null
    ...
    cell.push(
      <Form.Check
        ...
        disabled={isDisabled}
        checked={
          typeof child.props.checked !== "undefined"
            ? child.props.checked(data)
            : null
        }
  ...
  static addLinkButtonCell(data, child) {
    ...
    if ("data" in child.props && "dataKey" in child.props) {
      ...
      if (keys != null) {
        let filteredData = child.props.data.filter((data) =>
  ...
  static addButtonCell(data, index, child) {
    ...
    let isLocked =
      child.props.locked ||
      (child.props.isLocked != null && child.props.isLocked(index));
    ...
    let showEditButton =
      child.props.showEditButton !== null &&
      child.props.showEditButton !== undefined
    ...
    if (child.props.exportButton) {
      cell.push(
        this.getBtn(childkey, "download", "Export", index, child.props.onExport)
      );
    }
    ...
    if (child.props.signalButton) {
      cell.push(
        this.getBtn(
          childkey,
          "wave-square",
          "Autoconfigure Signals",
          index,
          child.props.onAutoConf,
          isLocked
        )
      );
    }
    ...
    if (child.props.downloadAllButton) {
      cell.push(
        this.getBtn(
          ...
          "Download All Files",
          index,
          child.props.onDownloadAll
        )
      );
    }

    let showDeleteButton =
      child.props.showDeleteButton !== null &&
      child.props.showDeleteButton !== undefined
          ...
          {this.state.rows.map((row, rowIndex) => (
            <tr key={rowIndex}>
              {row.map((cell, cellIndex) => {
                let isCellInlineEditable =
                  children[cellIndex].props.inlineEditable === true;
                ...
                let evtHdls = isCellInlineEditable
                  ? {
                      ...
                      onCellFocus: () =>
                        this.onCellFocus({ cell: cellIndex, row: rowIndex }),
                ...
                return (
                  ...
                  >
                    {this.state.editCell[0] === cellIndex &&
                    this.state.editCell[1] === rowIndex ? (
                      <Form.Control
                        as="input"
                        type={children[cellIndex].props.inputType}
                        value={cell}
                        onChange={(event) =>
                          children[cellIndex].props.onInlineChange(
                            event,
                            rowIndex,
                            cellIndex
                          )
